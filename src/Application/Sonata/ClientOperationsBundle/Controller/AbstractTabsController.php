...
class AbstractTabsController extends Controller
{
    ...
    /**
     * @param $reflect \ReflectionClass
     * @param $pr array
     */
    function getProperties($reflect, &$pr){
        $props   = $reflect->getProperties();

        $reflect = $reflect->getParentClass();
        if ($reflect){
            $this->getProperties($reflect, $pr);
        }
    ...
    public function listAction()
    {
        ...
        $p = array();
        foreach ($this->_config_excel as $tab){
            $class = 'Application\Sonata\ClientOperationsBundle\Entity\\'.$tab['entity'];

            //echo "<hr /><h3>".$tab['entity']."</h3>";
        ...
        $ent = array();
        foreach($p as $prop=>$entities){
            $key = 0;
            if (!array_diff($a, $entities)){
                $key += 1;
            }

            if (!array_diff($v, $entities)){
                $key += 2;
            }

            if (!array_diff($d, $entities)){
                $key += 4;
            }
